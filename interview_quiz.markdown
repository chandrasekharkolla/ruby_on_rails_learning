...
##### How to test a method that input a string output a random order string, how to fix the problem

```ruby
methodA('abcd') #=> 'abdc'/'adcb'/'dacb' it should not ordered and each time the output should be different 
```
...
1. controller-Keep code in controller as simple as the code generated by scarfold
2. Don't use low-level database in controller layer, like transactions
3. Session--stores references instead of instances
